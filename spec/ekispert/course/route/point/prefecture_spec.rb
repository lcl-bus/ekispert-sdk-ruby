require 'spec_helper'
require 'helper/util_helper'

RSpec.describe Ekispert::Course::Route::Point::Prefecture do
  include UtilHelper

  let(:xml) { read_xml('course/simple_search.xml') }
  let(:parsed_xml) { Ekispert::Client.send(:parse_xml, xml) }
  let(:course_list) { Ekispert::Course.send(:to_course, parsed_xml) }
  let(:prefecture) { course_list[0].route_list[0].point_list[0].prefecture_list[0] }

  describe '#name_list' do
    it 'return Array and contains Course::Route::Point::Prefecture::Name instance' do
      expect(prefecture.name_list[0].class).to eq Ekispert::Course::Route::Point::Prefecture::Name
    end
  end
  describe '#name' do
    it 'should return "東京都"' do
      expect(prefecture.name).to eq '東京都'
    end
  end
end
